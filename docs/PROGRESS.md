...
**Last updated:** 14 September 2026, end of M0a.
...
(clean), and `flutter build apk --debug` → `build/app/outputs/flutter-apk/app-debug.apk`.

**Not verified:** launching on a handset. No Android device was connected during the session.
This is the one M0a criterion still owed — see the open items below.
...
1. **Launch on a handset.** M0a was never run on a device. Do this at the top of the next
   session with a phone attached: `flutter run`. Expect a blank `--paper` screen.
2. **Developer Mode on Windows.** `flutter pub get` warns that plugin builds need symlink
...
3. **`speech_to_text` applies the Kotlin Gradle Plugin,** and the build warns that *"future
...
4. **`applicationId` is still `com.example.chit`.** Fine for debug, rejected by Play. It
   needs a real reverse-domain id, which is the owner's call, and changing it also means
   renaming the Kotlin package directories under `android/app/src/main/kotlin/`. Do it before
   the first release build, not after.
5. **`sqlite3_flutter_libs` resolves to `0.6.0+eol`,** pulled in by `drift_flutter 0.3.1`.
   The `+eol` marker is upstream's. It works; check for a successor when M1 starts.
6. **Font bundle is ~1.8 MB,** of which Noto Serif Devanagari is 758 KB for the single
   चित्त mark. PACKAGES.md already suggests subsetting before shipping; this is the file
   that makes it worth doing.
7. **README §8.2 (re-transcription) and §8.3 (does Today carry enough rhythm) are still
