...
void main() {
  group('which tabs are drawn — ADR-109', () {
    test('find is not one of them until a tag exists', () {
      expect(ChitRoute.drawnWhen(findGoesSomewhere: false), <ChitRoute>[
        ChitRoute.today,
        ChitRoute.past,
      ]);
    });
    ...
    test('and is once one does', () {
      expect(ChitRoute.drawnWhen(findGoesSomewhere: true), ChitRoute.values);
    });

    test('the order never changes, so find arrives on the right', () {
      expect(
        ChitRoute.drawnWhen(findGoesSomewhere: true).take(2),
        ChitRoute.drawnWhen(findGoesSomewhere: false),
      );
    });
